...
export function BasketList() {
  ...
  const { toggleBasket, isBasketVisible } = useContext(CustomContext);
  const total = orders.reduce(
    (acc, item) => acc + item.price * item.quantity,
    0
  );
  const basketClass = isBasketVisible ? "basket basket--visible" : "basket";
  return (
    <div className={basketClass}>
      <ul className="basket__list collection">
        ...
        {orders.length ? (
          orders.map((item) => <BasketItem key={item.mainId} {...item} />)
        ) : (
          <h5>Empty</h5>
        )}
